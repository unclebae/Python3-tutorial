...
# Python(?=!) 
#  - "Python"과 매치한다. 만약 이후 오는 값이 ! 인경우 매치된다. 
# Python(?!!)
#  - "Python"과 매치한다. 이후에 오는 값이 !가 아닌경우 매치된다. 

# Special Syntax with Parenteses
# R(?#comment)
#  - "R"과 매치한다. 나머지는 모두 커멘트이다. 
# R(?i)uby
#  - 케이스에 상관없이 R이후의 값은 uby가 되면 매치된다. 
# R(?i:uby)
#  - 위와 동일
# rub(?:y|le)
#  - \1 백 레퍼런스 없이 그룹을 만든다. 
